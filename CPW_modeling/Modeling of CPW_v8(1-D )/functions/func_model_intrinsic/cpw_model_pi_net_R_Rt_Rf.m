function [ s_param_model] = cpw_model_pi_net_R_Rt_Rf(freq,RLGC_0123_fit,RLGC_f_0123_fit,cpw_len,num_pi)
% use num_pi sections to model the cpw,the values of the RLGCs and RLGC_fs come from
% the (L_0123_fit,R_0123_fit,G_123_fit,C_0123_fit) * cpw_len
% the lump parameters of the port is included
% output: S parameters of the model.

R_0123_fit = RLGC_0123_fit(:,1);
L_0123_fit = RLGC_0123_fit(:,2);
G_0123_fit = RLGC_0123_fit(:,3);
C_0123_fit = RLGC_0123_fit(:,4);
% f_RL = f_fit(1);
% f_GC = f_fit(2);
f_RL = (R_0123_fit(2:4)./L_0123_fit(2:4))./(2*pi);
f_GC = (G_0123_fit(2:4)./C_0123_fit(2:4))./(2*pi);
Rf = RLGC_f_0123_fit(:,1);
Lf = RLGC_f_0123_fit(:,2);
Gf = RLGC_f_0123_fit(:,3);
Cf = RLGC_f_0123_fit(:,4);
...
G_model_len =             G_0123_len(2)* (1-1./(1+(freq./f_GC(1)).^2)) + ...,
...
Yf_model = (1i * 2*pi *freq).*Cf + Gf;
Zf_model = (1i * 2*pi *freq).*Lf + Rf;

ABCD_param_model = zeros(2,2,length(freq));
for k =1:length(freq)
    
    ABCD_f = [1+Zf_model(k)*Yf_model(k),2*Zf_model(k)+Zf_model(k)*Zf_model(k)*Yf_model(k);....
              Yf_model(k),1+Zf_model(k)*Yf_model(k)];
%     ABCD_f= [1,2*Zf_model(k);0,1];
    ABCD_Y_left_right =   [1,0;Y_model_len(k)./(2*num_pi),1];
    ABCD_Y_medium =   [1,0;Y_model_len(k)./num_pi,1];
    ABCD_Z = [1, Z_model_len(k)./num_pi;0,1];
    ABCD_param_model(:,:,k) = ABCD_f*ABCD_Y_left_right * ...,
                        ((ABCD_Z*ABCD_Y_medium)^(num_pi-1))* ...,
                        ABCD_Z*ABCD_Y_left_right*ABCD_f;
