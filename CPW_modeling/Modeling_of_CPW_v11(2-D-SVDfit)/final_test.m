...
%% HFSS simulation
S_sim_total = cpw_cell{space-2,width-3}.files_info(round((len-100)/50)).Sparam_total;
S_sim_i = cpw_cell{space-2,width-3}.files_info(round((len-100)/50)).Sparam_i;
...
% plot_RLGC_triple(freq,RLGC_hspice,RLGC_total,RLGC_i,{'hspice','sim\_total','sim\_i'},'RLGC: hspice vs sim_total',{'-r','--b','-g'});



plot_RLGC_double(freq,RLGC_hspice,RLGC_i,{'model','EM'},'RLGC: model vs EM',{'-r','--b'});
plot_Sparam_double_Real_Imag( freq,S_hspice,S_sim_i,{'model','EM'},'model VS EM','off');
