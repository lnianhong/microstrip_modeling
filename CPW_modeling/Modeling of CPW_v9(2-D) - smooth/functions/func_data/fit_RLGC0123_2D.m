...
if strcmp(first_var,'space')
    ...
    coef_s = zeros(length(Width_array),4,16);% 16 means RLGC0123,coef of s;coef(s) = f(w)
    ...
    for k=1:16% 16 means RLGC0123
        ...
        [ fit_result] = fit_RLGC0123( W_interp',coef_s_index_interp); %coef(s) = f(w)
        for m=1:4
...
if strcmp(first_var,'width')
    % fit the RLGC 0123
    coef_2D = zeros(4,4,16);
    coef_w = zeros(length(Space_array),4,16);% 16 means RLGC0123
    ...
    for k=1:16% 16 means RLGC0123
        ...
        [ fit_result] = fit_RLGC0123( S_interp',coef_w_index_interp);
        for m=1:4
            coef_2D(:,m,k) = [fit_result(m).cfit.p1;fit_result(m).cfit.p2;...
                fit_result(m).cfit.p3;fit_result(m).cfit.p4];
        end
