function plot_RLGCerr_2Dvar( err_RLGC_ave,err_RLGC_max,...
                                               Width_array,Space_array,...
                                               var_str,index  )
if strcmp(var_str,'space')
    err_ave = permute(err_RLGC_ave(:,index,:),[3 1 2]);
    err_max = permute(err_RLGC_max(:,index,:),[3 1 2]);
    x_str = 'Space [\mum]';
    var = Space_array;
    cmp = ['Width=',num2str(Width_array(index))];
elseif strcmp(var_str,'width')
    err_ave = permute(err_RLGC_ave(index,:,:),[3 2 1]);
    err_max = permute(err_RLGC_max(index,:,:),[3 2 1]);
    x_str = 'Width [\mum]';
    var = Width_array;
