function  plot_RLGC_2D_scalable_var( RLGC_0123_mat,coef_2D,Width_array,Space_array,var_str,index )
...
if strcmp(var_str,'space')
    ...
    Space_tmp = repmat(var_interp',1,4).^[3 2 1 0];
    Width_tmp = repmat((Width_array(index))',1,4).^[3 2 1 0];
...
elseif strcmp(var_str,'width')
    ...
    RLGC_0123_scalable = zeros(16,length(var_interp));
    Space_tmp = repmat((Space_array(index))',1,4).^[3 2 1 0];
    Width_tmp = repmat(var_interp',1,4).^[3 2 1 0];
    for k = 1:16 % 16 means RLGC0123
        RLGC_0123_scalable(k,:) =( Space_tmp*coef_2D(:,:,k)*(Width_tmp)');
    end
