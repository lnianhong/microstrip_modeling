clc;clear;close all ;
dbstop if error;
data_name = 'tmp_data/cpw_2D_W4-10_S2_10';
load([data_name,'.mat']);
...
poly_n = 4;
if poly_n == 4
    coef_2D = fit_RLGC0123_2D_poly4( Width_array,Space_array,RLGC_0123_mat,'space');
    Space_tmp = repmat(Space_array',1,5).^[4 3 2 1 0];
    Width_tmp = repmat(Width_array',1,5).^[4 3 2 1 0];
else
    coef_2D = fit_RLGC0123_2D( Width_array,Space_array,RLGC_0123_mat,'space');
...
%% generate hspice list

%% plot the scalable result
if poly_n == 4
    plot_RLGC_2D_scalable_var_poly4( RLGC_0123_mat,coef_2D,Width_array,Space_array,'space',1 );
    plot_RLGC_2D_scalable_var_poly4( RLGC_0123_mat,coef_2D,Width_array,Space_array,'width',3 );
else
    plot_RLGC_2D_scalable_var( RLGC_0123_mat,coef_2D,Width_array,Space_array,'space',1 );
    plot_RLGC_2D_scalable_var( RLGC_0123_mat,coef_2D,Width_array,Space_array,'width',3 );
end
mesh_RLGC_2D_double( RLGC_0123_mat,coef_2D,Width_array,Space_array,1);
