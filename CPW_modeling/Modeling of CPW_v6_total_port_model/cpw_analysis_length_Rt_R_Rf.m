% 考虑分端口效应，两种模型（端口效应有GC和没有GC），两种模型代表两种回归方式；两种模型验证方法（整体和本征+叠加端口）
clc;clear;close all
dbstop if error;
addpath('./function')
...
if strcmp(mode{2},'add')
    RLGC_model_test_total = [R_model_test_total+2*Rf_sim./len_test,L_model_test_total+2*Lf_sim./len_test,...
                             G_model_test_total+2*Gf_sim./len_test,C_model_test_total+2*Cf_sim./len_test];
end
