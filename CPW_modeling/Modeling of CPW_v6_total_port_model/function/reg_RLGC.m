...
for k = 1:files_num
    ...
    [R_sim_total(:,k),L_sim_total(:,k),G_sim_total(:,k),C_sim_total(:,k),...
        files(k).gamma_total,files(k).Z0_total] = ...,
        S_2_RLGC(s_params,files(k).freq,len(k)); % the unit length is meter!
    files(k).Sparam_sim_total = s_params;
    files(k).RLGC_total = [R_sim_total(:,k),L_sim_total(:,k),G_sim_total(:,k),C_sim_total(:,k)];
end
