function [ RLGC_0123_fit,RLGC,RLGC_model,gamma,gamma_model,f_fit_RL,f_fit_GC,Err_RLGC_fit,Err_RLGC_model,Err_s,Err_alpha_beta] ...,
              = cpw_single_v2( file_name,len,freq_min_max,f_123_mat,lambda,num_pi,disp_sw )
% 比CPW_single()多几个返回值-RLGC,RLGC_model,gamma,gamma_model
...
Err_RLGC_fit=disp_RLGC_err( RLGC,RLGC_fit,f_fit_RL,f_fit_GC,'Summary of RLGC fit');

%% use equations to calculate the s parameters of the equivalent model
RLGC_0123_fit = [R_0123_fit,L_0123_fit,G_0123_fit,C_0123_fit];
...
Err_RLGC_model = disp_RLGC_err( RLGC,RLGC_model,f_fit_RL,f_fit_GC,'Summary of RLGC model');
if strcmp(disp_sw,'on')
...
Err_s = disp_Sparam_error( s_params,S_model_cal,'Error of S parameters');
if strcmp(disp_sw,'on')
    [Err_alpha,Err_beta ]=plot_gamma_double(freq,gamma,gamma_model,{'sim','model'},'alpha and beta: sim vs model',{'-r','--b'} );
    Err_alpha_beta  = disp_alpha_beta_err( Err_alpha,Err_beta,'Error of alpha & beta' );
else
    Err_alpha_beta = disp_alpha_beta_err_v2( gamma,gamma_model,'Error of alpha & beta' );
end
