function [files, RLGC_sim,RLGC_f_sim] = reg_RLGC_unsmooth( files,freq_min,freq_max,num_freq_point,mode)
%give a group of files(only the length is different)
% return the intrinsic RLGC and the lumped port parameters RLGC_f
if nargin < 2
    freq_min = 0.5e9;
end
if nargin < 3
...
for k = 1:files_num
    ...
    %smooth the sim data
%     [ files(k).RLGC_total_fit,~,~ ] = RLGC_2_0123( files(k).RLGC_total,files(k).freq);
%     files(k).Sparam_sim_total = s_params;
end
% after smooth
% RLGC_sim_total = [files.RLGC_total_fit];
