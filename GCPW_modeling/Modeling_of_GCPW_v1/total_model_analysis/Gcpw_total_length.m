...
clear;close all
dbstop if error;
len = [125 150 200 250 300 350 400];%W ==5
len_pi = 12.5*1e-6;
space = 10;
width = 4;
totalmodel  = new_files_info_total();
for k = 1:length(len)
    totalmodel(k).name=sprintf('../S_parameters_sim/GCPW_test/L%dW%dS%d.csv',len(k),width,space);
%     totalmodel(k).name = sprintf('../S_parameters_sim/all/L%dW%dS%d.csv',len(k),width,space);
    ...
    totalmodel(k) = model_total_baseinfo( totalmodel(k));
